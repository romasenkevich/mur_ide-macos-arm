...
Cocoa_GL_GetDrawableSize(_THIS, SDL_Window * window, int * w, int * h)
{
    ...
    CGFloat scale = 1.0;
    if (windata->nswindow) {
        scale = [windata->nswindow backingScaleFactor];
    }
    ...
    if (window->flags & SDL_WINDOW_ALLOW_HIGHDPI) {
        ...
        if ([contentView respondsToSelector:@selector(convertRectToBacking:)]) {
            ...
            /* On external Qt NSView, convertRectToBacking may still return logical
               size; fall back to explicit backing scale if needed. */
            if (scale > 1.0 &&
                viewport.size.width <= [contentView bounds].size.width + 0.5 &&
                viewport.size.height <= [contentView bounds].size.height + 0.5) {
                viewport.size.width = [contentView bounds].size.width * scale;
                viewport.size.height = [contentView bounds].size.height * scale;
            }
        }
    } else if (scale > 1.0 && !windata->created) {
        /* External windows can still be Retina-backed; for embedded NSView use
           backing scale to report the true drawable size. */
