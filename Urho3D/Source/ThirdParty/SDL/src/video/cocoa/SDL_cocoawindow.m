...
{ @autoreleasepool
{
    ...
    id cocoaObject = (id)data;
    NSString *title;

    /* Qt6 on macOS provides NSView* (QNSView) via winId(), while old SDL
       expects NSWindow*. Support both to avoid selector crashes. */
    if (cocoaObject && [cocoaObject isKindOfClass:[NSView class]]) {
        contentView = (NSView *)cocoaObject;
        nswindow = [contentView window];
    } else if (cocoaObject && [cocoaObject isKindOfClass:[NSWindow class]]) {
        nswindow = (NSWindow *)cocoaObject;
        contentView = [nswindow contentView];
    }

    if (!nswindow || ![nswindow isKindOfClass:[NSWindow class]]) {
        return SDL_SetError("Cocoa_CreateWindowFrom expected NSWindow or NSView");
    }

    /* On Retina displays with external NSView windows (Qt), explicitly request
       best-resolution OpenGL surface to keep SDL drawable size in sync. */
    if (contentView && (window->flags & SDL_WINDOW_ALLOW_HIGHDPI)) {
        if ([contentView respondsToSelector:@selector(setWantsBestResolutionOpenGLSurface:)]) {
            [contentView setWantsBestResolutionOpenGLSurface:YES];
        }
    }
    ...
    if (SetupWindowData(_this, window, nswindow, SDL_FALSE) < 0) {
        return -1;
    }

    windowData = (SDL_WindowData *)window->driverdata;
    if (windowData && contentView) {
        windowData->sdlContentView = contentView;
    }
    return 0;
}}
