...
SettingsController::SettingsController()
{
    ...
    if (os == "darwin") {
        if (!isValidPath(m_resourcePath)) {
            // App bundle layout: .../MyApp.app/Contents/MacOS -> Resources.
            m_resourcePath = path.absolutePath() + "/../Resources/";

            // Dev/default layout when running from build directory.
            if (!isValidPath(m_resourcePath)) {
                m_resourcePath = path.absolutePath() + "/../resources/";
            }

            // Dev/default layout when running from source directory.
            if (!isValidPath(m_resourcePath)) {
                m_resourcePath = path.absolutePath() + "/resources/";
            }
        }
